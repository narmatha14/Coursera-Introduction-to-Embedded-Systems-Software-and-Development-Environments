...
void main() {
  ...
  sort_array(test, arr_length); // we need to sort the array to find its median correctly
...
void print_statistics(unsigned char arr[], unsigned int arr_length)
{
  ...
  printf("Mean of test array = %i\n", find_mean(arr, arr_length));
  printf("Median of test array = %i\n", find_median(arr, arr_length));
}
...
void sort_array(unsigned char arr[], unsigned int arr_length)
{
  ...
  unsigned int inner_idx; // inner loop index
  unsigned char temp;
